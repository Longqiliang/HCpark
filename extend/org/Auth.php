...
//        dump($authList);
    ...
    protected function getAuthList($uid, $type) {
        ...
        if( $this->_config['auth_type'] == 2 && !empty(session('_auth_list_'.$uid.$t))) {
        ...
        // 读取用户组所有权限规则
        $rules = Db::table($this->_config['auth_rule'])->where('type', 'in', $type)->where('status', 1)->where('id', 'in', $ids)->field('condition,name')->select();
        // 循环规则，判断结果。
        $authList = array();   //
    ...
    protected function getUserInfo($id) {
        ...
        if( !isset($userInfo[$id]) ){
            $userInfo[$id] = Db::table($this->_config['auth_user'])->where('id', $id)->find();
        }
        return $userInfo[$id];
    }
