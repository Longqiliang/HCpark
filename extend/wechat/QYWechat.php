...
class QYWechat
{
	...
	const CONVERT_TO_OPENID     = '/user/convert_to_openid?';
    const CONVERT_TO_USERID     = '/user/convert_to_userid?';
...
<Nonce><![CDATA[%s]]></Nonce>
	...
	public function getJsSign($url, $timestamp=0, $noncestr='', $appid=''){
	    ...
	    $sign = $this->getSignature($arrdata, 'md5');
	    if (!$sign)
	        return false;
	    $signPackage = array(
	            "appid"     => $this->appid,
    ...
    public function convertToOpenId($data){
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_post(self::API_URL_PREFIX.self::CONVERT_TO_OPENID.'access_token='.$this->access_token,self::json_encode($data));
        if ($result)
        {
            $json = json_decode($result,true);
            if (!$json || !empty($json['errcode']) || $json['errcode']!=0) {
    ...
    public function convertToUserId($data){
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_post(self::API_URL_PREFIX.self::CONVERT_TO_USERID.'access_token='.$this->access_token,self::json_encode($data));
        if ($result)
        {
            $json = json_decode($result,true);
            if (!$json || !empty($json['errcode']) || $json['errcode']!=0) {
