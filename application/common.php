 ...
 * @return bool|static
 */
function get_images_path($images_id, $field = 'path'){
    if(empty($images_id)){
        return '';
    }
    $picture = \app\admin\model\Picture::get($images_id);
    return empty($field) ? $picture : $picture[$field];
}
function get_video($video_id, $field = 'name'){
    if(empty($video_id)){
        return '';
    }
    $video = \app\admin\model\Video::get($video_id);
    return empty($field) ? $video : $video[$field];
}
/**
 * 字符串截取，支持中文和其他编码
 * @static
